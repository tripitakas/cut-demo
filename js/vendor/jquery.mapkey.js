...
(function($) {
  ...
  $.fn.mapKey.storeBinding = function(key, value, e){
    if(typeof key == "number"){
      ...
      var keys = key.split('+');
      var code = $.fn.mapKey.keys[keys[keys.length - 1]];

      //support composed keys such as ctrl+shift+key, by rhcad on 2018.9.17
      keys[keys.length - 1] = '';
      keys = keys.join('+');
      ...
      if(typeof code == "number"){
        ...
        if(e == "up"){
          $.fn.mapKey.bindings.up[keys + code.toString()] = value;
        }else{
          $.fn.mapKey.bindings.down[keys + code.toString()] = value;
        }
      }else if(typeof code == "object"){
        //add multiple bindings for each version of the key
        $.each(code, function(i, v){
          if(e == "up"){
            $.fn.mapKey.bindings.up[keys + v.toString()] = value;
          }else{
            $.fn.mapKey.bindings.down[keys + v.toString()] = value;
          }
        });
      } 
    }
  ...
  //parse composed keys such as ctrl+shift+key, by rhcad on 2018.9.17
